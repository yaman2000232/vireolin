<template>
  <v-container v-if="authStore.role === 'customer'">
    <div class="d-flex justify-center mb-5 ga-5 mt-5">
      <v-chip color="primary" size="x-large" class="text-h5 font-weight-bold px-6 py-3 mb-6">
    ...
    <div v-if="ourRequestStore.loading">
      <v-skeleton-loader type="card" class="mx-auto" max-width="400"></v-skeleton-loader>
    </div>
    ...
    <div v-else-if="ourRequestStore.bookings.length === 0" class="d-flex flex-column align-center justify-center my-12">
      ...
      <span class="text-h5 font-weight-bold text-grey-darken-1">{{ $t('myRequests.noBookings') }}</span>
    </div>

    <!-- Bookings list -->
    <v-row v-else align="stretch">
      <v-col v-for="booking in ourRequestStore.bookings" :key="booking.id" cols="12" sm="6" md="4">
        <v-card class="pa-4 rounded-lg elevation-3 d-flex flex-column position-relative h-100 pa-5">
          <h3 class="text-h6 mb-2">
            {{ booking.service_type?.title || $t('myRequests.defaultServiceTitle') }}
          </h3>

          <v-chip :color="getStatusColor(booking.status)" dark class="ma-2">
            <v-icon start>{{ getStatusIcon(booking.status) }}</v-icon>
            {{ $t(`myRequests.status.${booking.status}`) }}
          </v-chip>

          <p class="mb-1"><strong>{{ $t('myRequests.message') }}:</strong> {{ booking.message }}</p>

          <v-btn color="primary" class="mt-auto" @click="openDetailsDialog(booking.id)">
      ...
      <v-card>
        ...
        <v-card-text class="pa-6">
          ...
          <div v-else-if="!ourRequestStore.selectedBooking" class="text-grey text-h6">
            {{ $t('myRequests.dialog.noDetails') }}
          </div>
          <div v-else>
            <v-simple-table>
              <tbody>
                <tr>
                  <td class="font-weight-bold text-h6">{{ $t('myRequests.dialog.bookingId') }}</td>
                  <td class="text-h6">{{ ourRequestStore.selectedBooking.id }}</td>
                </tr>
                <v-divider></v-divider>

                <tr>
                  <td class="font-weight-bold text-h6">{{ $t('myRequests.dialog.service') }}</td>
                ...
                <tr>
                  <td class="font-weight-bold text-h6">{{ $t('myRequests.dialog.description') }}</td>
                  <td class="text-h6">{{ ourRequestStore.selectedBooking.service_type.description }}</td>
                </tr>
                <v-divider></v-divider>

                <tr>
                  <td class="font-weight-bold text-h6">{{ $t('myRequests.dialog.status') }}</td>
                  <td>
                    <v-chip :color="getStatusColor(ourRequestStore.selectedBooking.status)" size="large" dark>
                      ...
                      {{ $t(`myRequests.status.${ourRequestStore.selectedBooking.status}`) }}
                    </v-chip>
                  </td>
                </tr>
                <v-divider></v-divider>
                ...
                <tr>
                  <td class="font-weight-bold text-h6">{{ $t('myRequests.dialog.emailVerified') }}</td>
                  <td>
                    <span :class="ourRequestStore.selectedBooking.user.email_verified_at ? 'text-success text-h6' : 'text-error text-h6'">
                      {{ ourRequestStore.selectedBooking.user.email_verified_at ? $t('myRequests.dialog.verified') : $t('myRequests.dialog.notVerified') }}
                    </span>
                  </td>
                </tr>
                <v-divider></v-divider>
                ...
                <tr>
                  <td class="font-weight-bold text-h6">{{ $t('myRequests.dialog.message') }}</td>
    ...
    <v-dialog v-model="showCancelDialog" max-width="400">
      <v-card>
        <v-card-title class="text-h6 font-weight-bold">
          ...
          {{ $t('myRequests.cancelDialog.title') }}
        </v-card-title>

        <v-card-text>
          {{ $t('myRequests.cancelDialog.message') }}
        </v-card-text>

        <v-card-actions class="justify-end">
          <v-btn text color="grey" @click="closeCancelDialog">{{ $t('myRequests.cancelDialog.no') }}</v-btn>
          <v-btn color="error" @click="confirmCancel">{{ $t('myRequests.cancelDialog.yes') }}</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
